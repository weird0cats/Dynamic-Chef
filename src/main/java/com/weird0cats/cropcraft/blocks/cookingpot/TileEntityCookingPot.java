...
public class TileEntityCookingPot extends TileEntity implements ITickable 
{
   ...
   {
      if (this.itemStackHandler.getStackInSlot(INPUT_SLOTS_START+0).isEmpty()
   ...
   public void update()
   {
      ...
      if (this.burnFuel(isCooking && hasRecipe) && isCooking && hasRecipe)
      {
         ++this.cookTime;
   ...
   protected void cook()
   {
      if (this.canCook())
      {
         ...
         if (recipe.matches(inputs))
         {
            ...
            itemStackHandler.extractItem(INPUT_SLOTS_START+0, consume[0], false);
            itemStackHandler.extractItem(INPUT_SLOTS_START+1, consume[1], false);
            itemStackHandler.extractItem(INPUT_SLOTS_START+2, consume[2], false);
            itemStackHandler.extractItem(INPUT_SLOTS_START+3, consume[3], false);
            itemStackHandler.extractItem(INPUT_SLOTS_START+4, consume[4], false);
            itemStackHandler.extractItem(INPUT_SLOTS_START+5, consume[5], false);
         }
      }
   }
}